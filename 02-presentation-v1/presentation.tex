...
% slide 4
\begin{frame}
% maybe include the approximate comp complexit of the analytical 
% solution
...
In a usuall case, regularization parameter is used to "punish" big values of a model parameters $\vec{\theta}$, that lead to the overfitting, of the neural network. In our case, we use it as a constrait for the model. 
...
\begin{frame}{Cross-validation (CV) algorithm}
    ...
    \vspace{0.5cm}
    2) Repeate k times for a chosen $\gamma$ value: \\
...
\begin{frame}{Bootstrapping parameters $\vec{\lambda}$}
    ...
    \quad - Choose 80\% of the setted data and use it for training and testing the model. \\
