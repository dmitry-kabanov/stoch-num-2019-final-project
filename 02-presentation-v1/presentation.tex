...
\begin{frame}{Porblem optimization}
    
As an optimization algorithm, we use Limited-memory
Broyden-Fletcher-Goldfarb-Shanno (BFGS) algorithm. \\
It is one of the quasi-Newtone methods that approximate the Hessian
matrix instead of it's computation. \\
In this method, from initial
guess $\vec{x_0}$, and an approximate matrix $\vec{B_0}$ we are
lookoing for the direction of optimization and iteratively
approximate $B$ until convergence.
...
\begin{frame}{Tensorflow}
    ...
    Neural network is a graph of variables and a mathematical operations. And Tensorflow is a package that allows us to set mathematical operations as an element of a graph and to make computations over the variables that are also a part of the graph. 
    
    \includegraphics[scale = 0.4]{02-presentation-v1/images/graph-example.png}
    
\end{frame}

\begin{frame}
    Therefore, in this work, using Tensorflow, we 
    \begin{enumerate}
        \item Set mathematical operations and placeholders for variables,
        \item set configurations of a graph as a neural network,
        \item process the computation.
    \end{enumerate}
\end{frame}
...
$2*10 + (10*10 + 10)*3 + 10*1 + 1 = 370$\\
    \item $3700$ observatons were used.
\end{enumerate}
...
Parameter $\gamma$ for noisy data: $u_{obs} = u + N(0, \sigma^2),$ $\sigma = 0.05 u.$ 
...
Comparison between the exact and the predicted solutions for the noisy data. $u_{obs} = u + N(0, \sigma^2),$ $\sigma = 0.05 u.$ \\

\centering
\includegraphics[scale = 0.43]{02-presentation-v1/images/burgers-exact-pred-noisy.png}
