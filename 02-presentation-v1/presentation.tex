...
The exact solution is: $$u(t, x) = \frac{\pi \lambda_2}{\pi \lambda_2 e^{\{\pi^2 \lambda_2 t\}} - (e^{\{\pi^2 \lambda_2 t\}} - 1) cos(\pi x)}$$

\end{frame}

\begin{frame}{Burgers' equation neural network configuration}
    
In the neural network for this problem 3200 number of observations were used. As the number of them can be approximatelly calculated using the formula: $2*M^2$, where $M$ is the number of neural network parameters. Here we have 4 layers with 10 neurons in each, therefore $2*M^2 = 3200.$
This optimal number is chosen in order to avoid overfitting and underfitting.

\end{frame}

\begin{frame}{Observations}
 Below the exact solution is given on the uniform grid with 201 points in $x$ and 101 points in $t$. 
\begin{figure}
    \centering
...
\midrule
  1 & 19.43 & 31.82 & 25.32 & 118.02 & 79.21 & 74.33\\
  \hline
